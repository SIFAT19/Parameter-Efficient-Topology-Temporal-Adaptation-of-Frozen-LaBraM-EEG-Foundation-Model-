...
def cosine_scheduler(base_value, final_value, epochs, niter_per_ep, warmup_epochs=0,
    ...
    # Prevent warmup from exceeding total training iterations
...
def get_metrics(output, target, metrics=None, is_binary=True, threshold=0.5):
    """
    Compute evaluation metrics using the project's custom metrics.py.
    """

    if is_binary:
        y_prob = output.reshape(-1)
        y_true = target.reshape(-1).astype(int)
